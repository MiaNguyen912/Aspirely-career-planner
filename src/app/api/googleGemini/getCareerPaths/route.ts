...
export async function POST(request: NextRequest) {
  try {
    ...
    const { resume, major } = await request.json();
    ...
    // const prompt = `This is the major: "${major}"; this is the resume text: "${resume}".`;
    const prompt = `You are a career consultant. You are given a major of \"${major}\", and a resume \"${resume}\", and you need to analyze if the skills and experiences in the resume are relevant to any career pathways related to that major field. You should output a list of career objects with the following properties: id, name, averageSalary, demandIn5Years, percentageMenInTheWorkforce, percentageWomenInTheWorkforce.`;
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              percentageMenInTheWorkforce: { type: "number" },
              percentageWomenInTheWorkforce: { type: "number" },
            },
            required: ["id", "name", "averageSalary", "demandIn5Years", "percentageMenInTheWorkforce", "percentageWomenInTheWorkforce"],
          },
        },
      },
    });
