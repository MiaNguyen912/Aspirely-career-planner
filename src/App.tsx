...
function App() {
  ...
  const [showMainPlatform, setShowMainPlatform] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setShowMainPlatform(true);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 flex items-center justify-center p-4">
      ...
      <div className="bg-white/10 backdrop-blur-md p-8 rounded-xl border border-white/10 w-full max-w-md relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full bg-blue-600/80 hover:bg-blue-700/90 text-white py-3 px-6 rounded-lg font-medium transition-all duration-200 backdrop-blur-sm"
          >
            Submit Application
          </button>
        </form>
